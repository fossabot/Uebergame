...
function doProfile(%val) {
	if (%val) {
		...
		setMsgArea("Top","Profiler is recording data");
		...
		setMsgArea("Top","Profiler is ending record session. Writing file:" SPC %file, 3000);
...
function toggleProfile(%val) {
	if (%val) {
		if($ProfilerActive) {
			...
			setMsgArea("Top","Profiler have been stopped. Writing to file:" SPC %file, 3000);
			profilerDumpToFile(%file);
			...
			setMsgArea("Top","Profiler have been activated");
			profilerReset();
			profilerEnable(true);
		}
	}
