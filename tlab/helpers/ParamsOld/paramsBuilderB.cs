...
function buildParamsInterfaceB( %params ) {
	...
	while(%params.groupTitle[%gid] !$="") {
		...
		if (!isObject(%stackObj)) {
			warnLog("Invalid stack object for group:",%params.groupTitle[%gid],"Param building skipped");
			%gid++;
			continue;
		}
		...
		while (%params.groupFieldData[%gid,%fid] !$= "") {
			...
			%fieldWidget = %widgetsControl.findObjectByInternalName(%fieldType);
			if(!isObject(%fieldWidget)) {
				warnLog("Couldn't find widget for setting:",%fieldTitle,"This setting building is skipped");
				%fid++;
				continue;
			}
...
//------------------------------------------------------------------------------
//===============================================================================
function prepareParamsInterfaceB( %params ) {
