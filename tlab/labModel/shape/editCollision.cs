...
function LabModelColWindow::onWake( %this ) {
	%this-->colType.clear();
...
function LabModelColWindow::update_onShapeSelectionChanged( %this ) {
	...
	for ( %i = 0; %i < %objCount; %i++ )
		%this-->colTarget.add( LabModel.shape.getObjectName( %i ) );

	%this-->colTarget.setSelected( %this-->colTarget.findText( "Bounds" ), false );
}

function LabModelColWindow::update_onCollisionChanged( %this ) {
	...
	if ( %this-->colType.getText() $= "Convex Hulls" ) {
		%this-->hullContainer.setVisible( true );
		...
		%this-->hullContainer.setVisible( false );
	}
}

function LabModelColWindow::editCollision( %this ) {
	// If the shape already contains a collision detail size-1, warn the user
	// that it will be removed
	if ( ( LabModel.shape.getDetailLevelIndex( -1 ) >= 0 ) &&
		...
		LabMsgYesNo( "Warning", "Existing collision geometry at detail size " @
						 ...
						 "LabModelColWindow.editCollisionOK();", "" );
	} else {
		%this.editCollisionOK();
	}
}

function LabModelColWindow::editCollisionOK( %this ) {
	...
	LabModel.doEditCollision( %type, %target, %depth, %merge, %concavity, %maxVerts,
