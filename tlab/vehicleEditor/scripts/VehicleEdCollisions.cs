...
function VehicleEdColWindow::onWake( %this ) {
    %this-->colType.clear();
...
function VehicleEdColWindow::update_onShapeSelectionChanged( %this ) {
    ...
    for ( %i = 0; %i < %objCount; %i++ )
        %this-->colTarget.add( VehicleEditor.shape.getObjectName( %i ) );

    %this-->colTarget.setSelected( %this-->colTarget.findText( "Bounds" ), false );
}

function VehicleEdColWindow::update_onCollisionChanged( %this ) {
...
function VehicleEdColWindow::editCollision( %this ) {
    // If the shape already contains a collision detail size-1, warn the user
    // that it will be removed
    if ( ( VehicleEditor.shape.getDetailLevelIndex( -1 ) >= 0 ) &&
        ...
        LabMsgYesNo( "Warning", "Existing collision geometry at detail size " @
                          ...
                          "VehicleEdColWindow.editCollisionOK();", "" );
    } else {
        %this.editCollisionOK();
    }
}

function VehicleEdColWindow::editCollisionOK( %this ) {
    ...
    VehicleEditor.doEditCollision( %type, %target, %depth, %merge, %concavity, %maxVerts,
