...
function SceneCreatorWindow::createStatic( %this, %file ) {
    ...
    if(isFunction("getObjectLimit") && MissionGroup.getFullCount() >= getObjectLimit()) {
        ToolsMsgBoxOKBuy( "Object Limit Reached", "You have exceeded the object limit of " @ getObjectLimit() @ " for this demo. You can remove objects if you would like to add more.", "", "Canvas.showPurchaseScreen(\"objectlimit\");" );
        return;
    }

   devLog("Create Static ObjectGroup",%this.objectGroup);
...
function SceneCreatorWindow::addFolderIcon( %this, %text ) {
    ...
    %ctrl.iconBitmap = "tlab/gui/icons/base_assets/folder.png";
    %ctrl.text = %text;
    %ctrl.tooltip = %text;
