...
{
   filename = "art/sound/weapons/pistol";
...
datablock ProjectileData( ShotgunProjectile )
{
   ...
   lifetime            = 1000; // How long the projectile should exist before deleting itself
   fadeDelay           = 900; // Brief Amount of time, in milliseconds, before the projectile begins to fade out.

   bounceElasticity    = 0;
   bounceFriction      = 0;
   isBallistic         = false; // Causes the projectile to be affected by gravity "arc".
   bounceElasticity    = 0;
   bounceFriction      = 0;
   gravityMod          = 1;
...
datablock ShapeBaseImageData(ShotgunWeaponImage)
{
   ...
   stateSound[5]                    = ShotgunFireSoundList;
...
SmsInv.AddClip(ShotgunMag, "Shotgun Magazine", 1);

SmsInv.AllowAmmo("armor\tSoldier\t12");
SmsInv.AddAmmo(ShotgunAmmo, 6);
